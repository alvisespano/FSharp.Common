(*
 * F# Common Library
 * Arg.fs: argument parser (courtesy of Old F# PowerPack)
 * (C) 2007-2012 Alvise Spano' @ Universita' Ca' Foscari di Venezia
 *)
 
module FSharp.Common.Arg

type [< NoComparison; NoEquality >] ArgType = 
...
module Entry =
    let private arg argty f (name : string) help defopt =
        let pre =
            if name.Length = 0 then invalidArg name "empty argument name"
            else
                match name.[0] with
                    | '+' -> "+"
                    | _   -> "-"
        in
            ArgInfo (pre + (if name.Length = 1 then "" else pre) + name,
                     argty (fun x -> f x),
    ...
    let bool name f help def = 
        let noname = sprintf "no-%s" name
        in
            [|arg ArgType.Unit (fun () -> f true) name help def;
              arg ArgType.Unit (fun () -> f false) noname help def|]

    let synonyms names entry f help def = Array.map (fun name -> entry name f help def) names |> Array.concat

    let synonyms_no_def names entry f help = Array.map (fun name -> entry name f help) names |> Array.concat
